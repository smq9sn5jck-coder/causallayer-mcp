...
async function checkAndIncrementRl(ipHash: string | null, env: LeadsEnv): Promise<{ ok: true } | { ok: false; retry_after: number }> {
  if (!env.LEADS_RL || !ipHash) return { ok: true };
  const key = `rl:${ipHash}`;
  try {
    const raw = await env.LEADS_RL.get(key);
    const count = raw ? parseInt(raw, 10) || 0 : 0;
    if (count >= RL_MAX_PER_WINDOW) {
      return { ok: false, retry_after: RL_WINDOW_SECONDS };
    }
    await env.LEADS_RL.put(key, String(count + 1), { expirationTtl: RL_WINDOW_SECONDS });
    return { ok: true };
  } catch {
    return { ok: true };
  }
...
export async function handleLeads(
  ...
  // ─ Persist ──────────────────────────────────────────────────────────────
  if (!env.LEADS_DB) {
    // D1 not bound — return a structured error so the client can fall back to
    // Formspree without retrying. Should never happen in prod once the
    // wrangler.jsonc binding is added.
    return json({ ok: false, error: "leads_db_unbound" }, { status: 503 }, cors);
  }

  try {
    await env.LEADS_DB.prepare(
      `INSERT INTO leads (
         id, created_at_ms, email, company, role, sector, use_case,
         source, ref, cf_country, ip_hash, user_agent, turnstile, extras_json
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
      .bind(
        lead.id, lead.created_at_ms, lead.email, lead.company, lead.role,
        lead.sector, lead.use_case, lead.source, lead.ref, lead.cf_country,
        lead.ip_hash, lead.user_agent, lead.turnstile, lead.extras_json,
      )
      .run();
  } catch (e) {
    // Don't surface internal errors to the public; log via console.error so
    // Workers logs / observability picks it up.
    console.error("leads insert failed", String((e as Error)?.message || e));
